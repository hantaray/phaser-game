...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.createPortal();
        // creating the coins
        this.coins = this.map.createFromObjects('Coins', { name: 'Coin', key: 'coin' });
    ...
    addCollisions() {
        ...
        this.physics.add.overlap(this.player, this.enemiesGroup, this.player.enemyCollision
            .bind(this.player));
        this.physics.add.overlap(this.player, this.portal, this.loadNextLevel.bind(this, false));
        this.physics.add.overlap(this.coinsGroup, this.player, this.coinsGroup.collectCoin.
            bind(this.coinsGroup));
    ...
    createPlayer() {
        this.map.findObject('Player', (obj) => {
            if (this._NEWGAME && this._LEVEL === 1) {
                // there are 2 StartingPosition for the player - one for start game
                // and one for moving from Level2 
                // check right StartingPosition by type
